...
#DATASET 2  --> DAP (Gy.cm²) available

# Author:        Anna Peiró
# Date:          2026-07-22 
# ------------------------------
# Description:  Calculation of doses (mGy) from a database designated as 'dataset 2'.
# ------------------------------
...
#
#///  INPUT
#   - Data frame (df_input) containing DAP (Gy.cm²) and information on age, sex and ExamCode. Details below.
#
#///  OUTPUT
#   - Optional: 200 files containing the 200 realizations.
#   - df_ds2_doses_final.rds : includes the procedures, along with the mean and median of the 200 realizations.
# --------------
# Make sure that main_table_AM_coeff and df_input have consistent column types in the variables used for matching and calculations (factor, character, numeric, etc.)
#  --------------
...
# Load df_input
#Columns must include: PatientID, doe or CIncN (as procedure identification), age, sex, exam code, DAP (Gy.cm²)
...
# Load main table with coefficients (main_table_AM_coeff: "sex", "age", "Final_ExamCode", "AM_coeff") 
  #with coeff (factor) to be multiplied to the DAP (Gy.cm²) using parameters (sex, age, Final_ExamCode) 
...
# Function for AM dose calculation - Expected to be repeated the 200 realizations

dose_calculation_AM <- function(df, df_source) {
  df %>%
    left_join(
      df_source %>%
        group_by(sex, age, Final_ExamCode) %>%
        summarise(factores = list(AM_coeff), .groups = "drop"),
      by = c("sex", "age", "Final_ExamCode")
    ) %>%
    mutate(
      coeff = map_dbl(factores, ~ if (length(.x) > 0) sample(.x, 1) else NA_real_),
      AM_dose = DAP * coeff
    ) %>%
    select(-factores, -coeff)
}
...
for(i in 1:200){
  ...
  df_dose <- dose_calculation_AM(df, main_table_AM_coeff)
...
df_ds2_doses_final <- cbind(df0[,c("PatientID", "doe","DAP")],  # change "doe" to "CIncN" if needed
                            AM_median, AM_mean)

saveRDS(df_ds2_doses_final,"df_ds2_doses_final.rds")
